...
## 현재 단계
Phase 3 사용자 스토리 1 — 앱 컨셉·홈 로우파이 요구사항 명확화

## 진행 중
T022 — 앱 컨셉과 onboarding·home 로우파이 후보 사용자 선택 대기

## 마지막 완료 작업
T021 — US1 규칙 설정 흐름의 7개 로우파이 화면과 상태·접근성 가설을 작성함

## 다음 작업
사용자가 A·B·C 또는 조합을 선택하면 피드백을 로우파이에 반영하고 승인 여부를 기록함

## 차단 상태
없음

## 계획 갱신 필요
`DEC-015`·`DEC-016`에 따라 기존 단일 규칙 기준 `plan.md`, `data-model.md`, `tasks.md`와 저장소 구현을
다중 규칙 collection·활성 규칙 합집합 구조로 갱신해야 한다. 현재 컨셉·로우파이 검토는 진행할 수
있지만, 후속 core 구현을 재개하기 전에는 해당 구조 계획과 migration task를 먼저 확정한다.
