...
`닫기`만 남도록 정리하고 장식 아이콘·DEBUG 진단을 숨겼다. 한국어·영어 집중 UI 테스트 3건은
실패·skip 없이 통과했고 수정 서명 빌드를 iPhone 17에 설치했다. 다만 설치 직후 기기와 iPhone
...
확인했고, 실제 VoiceOver에서 발견한 읽기 순서 결함의 수정 빌드는 설치했다. 잠금 해제 뒤 수정된
실제 음성·초점 순서 재확인이 남았다.
