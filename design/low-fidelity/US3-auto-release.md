...
- 시간 종료 또는 신뢰 가능한 위치 이탈 뒤 기존 예정·비활성 홈 복귀
- 별도 완료 UI 없이 활성 표시와 guard를 제거하고 규칙 수정 control을 재활성화하는 동작
...
- 자동 해제 완료 전용 화면·배너·toast 또는 완료 전용 announcement
...
6. 신뢰 가능한 위치 이탈 또는 시간 종료가 확인되면 제한 앱 합집합을 다시 계산해 더 이상 필요한
   shield를 제거한다.
7. 별도 완료 UI를 표시하지 않고 활성 표시와 guard가 제거된 기존 예정·비활성 홈으로 돌아가
   `규칙 수정` control을 다시 활성화한다.
...
- 흐름: [활성 → 수정 시도 → 차단 안내 → 조건 종료 뒤 기존 홈 복귀](https://www.figma.com/design/cgw5wRUZRhUMWqEwrl0U04?node-id=126-1980)
...
- 자동 해제 뒤 전용 완료 화면·배너·toast를 표시하지 않고 활성 표시와 guard가 제거된 기존 홈
  상태를 사용한다.
- 기존 홈에서 `규칙 수정`을 다시 활성화하고 최신 규칙 상태로 편집을 시작한다.
...
- 활성·차단·해제 상태는 색상뿐 아니라 `RESTRICTION ACTIVE`, Alert 제목,
  조건 종료 뒤 활성 표시가 사라지는 상태 변화로 구분한다.
- Dynamic Type 확대에서는 Alert 설명이 자연스럽게 줄바꿈하며 중요 조건을 축약하지 않는다.
- Alert의 `확인`과 해제 뒤 `규칙 수정`은 최소 `44×44pt` touch target을 유지한다.
- Alert가 닫히면 focus는 변경을 시도한 control로 돌아간다.
- 조건 종료 시 완료 전용 focus 이동이나 announcement를 추가하지 않고 기존 홈의 읽기 순서를 사용한다.
- Reduce Motion에서도 별도 전환 화면 없이 기존 홈 상태만 갱신한다.

## 검토 체크리스트

- [x] 활성 중 편집·끄기·삭제를 같은 guard로 차단한다.
- [x] 차단 Alert가 실제 위치 또는 시간 종료 조건을 안내한다.
- [x] 시간 종료와 신뢰 가능한 위치 이탈이 같은 자동 해제 경로로 합류한다.
- [x] 위치 `unavailable`을 해제 조건으로 표현하지 않는다.
- [x] 자동 해제 뒤 별도 완료 UI 없이 기존 홈으로 돌아가 규칙 수정을 재활성화한다.
...
2026-08-24 사용자 피드백 반영 뒤 자동 해제 완료 전용 frame을 삭제하고, Figma wrapper와 남은 두
화면을 다시 렌더링해 검수했다. 전체 64개 text node는 `SF Pro Bold`,
...
| `2026-08-24` | `Codex` | 완료 전용 화면·배너·toast를 제거하고 조건 종료 뒤 기존 홈 복귀로 변경 | 사용자 변경 요청 |
