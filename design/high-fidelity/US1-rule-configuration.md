...
>
> 2026-08-27 사용자 승인 구현 보정: 신규 규칙의 시작 시각은 생성 시점의 현재 현지 시·분, 종료
> 시각은 15분 뒤로 초기화한다. `직접 입력` chip은 이름 입력 뒤 해당 이름으로 바뀌고 재탭하면 기존
> 이름을 유지한 field를 다시 연다. 홈 외부 세로 `ScrollView`는 제거하며, 저장 규칙이 하나면 card만
> 직접 표시하고 pager·page indicator·`좌우로 밀어 보기`는 규칙이 두 개 이상일 때만 표시한다. 이
> 동작은 기존 신규 초안 `80:1967`, 직접 입력 `81:2034`, 단일 규칙 홈 frame보다 우선한다.
>
> 2026-08-27 저장 장소 관리 보정: 직접 입력해 저장한 장소 chip에는 44pt 이상의 별도 삭제 행동을
> 제공하고 destructive 확인을 거친다. `집`·`회사`에는 삭제 행동이 없다. 참조 규칙이 있으면 장소를
> 보존하고 규칙 수를 안내하며, 미사용 장소만 삭제한다. 같은 규칙 편집 세션에서 위치 화면을 적용하지
> 않고 나갔다 다시 들어오면 마지막으로 누른 장소 chip의 선택 trait와 지도 핀을 유지한다.
...
7. 직접 입력 저장 장소 chip의 `xmark`를 누르면 장소 이름을 포함한 삭제 확인 Alert를 표시한다.
   참조 규칙이 있으면 `장소를 삭제할 수 없어요`와 규칙 수를 안내하고, 미사용 장소는 삭제 즉시
   chip에서 제거한다. 삭제 실패 시 chip을 유지하고 재시도 안내를 표시한다.
...
- 저장 장소 삭제 identifier: `locationPicker.savedPlace.<UUID>.delete`. 확인 문구는
  `<장소 이름>을 삭제할까요?`, 참조 차단 제목은 `장소를 삭제할 수 없어요`, 일반 실패 제목은
  `장소를 삭제하지 못했어요`를 사용한다.
...
| `2026-08-25` | 사용자 | `수정 승인` | 홈·장소·적용 CTA를 현재 Figma 하이파이에 맞추고 시간은 15분~12시간, 장소는 항상 보이는 집·회사·직접 입력과 10자·중복 금지로 보정 요청 | `T090`에서 SwiftUI·모델·테스트와 구현 인계에 반영 |
...
| `2026-08-27` | 사용자·`Codex` | 직접 입력 저장 장소에 확인형 삭제를 추가하고 규칙 참조 시 차단하며, 위치 화면 재진입 시 적용 전 선택을 유지하도록 상태 소유권을 보정했다. | 저장 장소 삭제·선택 유지 요청 |
