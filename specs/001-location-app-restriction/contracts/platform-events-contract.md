...
- 전체 권한 개요와 정상 승인 상태의 순차 확인은 설치 후 최초 실행의 온보딩에서 한 번만 수행한다.
- 온보딩 표시 여부를 영구 저장하고, 앱 프로세스를 종료하거나 기기를 재시작해도 다시 추정하지 않는다.
...
- 앱 소유 안내 화면의 시스템 alert·Settings 목업은 설명용 비대화형 요소이며 실제 권한 변경은
  시스템 prompt 또는 Settings에서만 수행한다.
