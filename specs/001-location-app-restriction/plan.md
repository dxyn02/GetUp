...
사용자가 선택한 요일·시간대와 500m 또는 1km 위치 조건이 모두 충족될 때 선택 앱에
시스템 제한 화면을 적용하는 로컬 iOS 앱을 구현한다. 메인 앱은 설정·권한·위치 상태를 관리하고,
시스템 일정 확장은 앱이 열려 있지 않을 때 시간 이벤트를 처리하며, 제한 화면 확장은 선택 앱
접근을 차단한다. 단일 규칙과 최근의 신뢰 가능한 위치 판정은 앱과 확장이 공유하는 보호된
저장소에 보관한다.

Phase 0에서 발견한 최소 일정 길이와 플랫폼 이벤트 지연은 `DEC-001`, `DEC-002`로 해결했다.
Phase 1은 단일 규칙 aggregate, 순수 제한 상태 머신, 보호된 공유 저장 계약, 시스템 이벤트 및
shield UI 계약으로 설계를 구체화한다.
...
**Constraints**: 완전 로컬 동작, 단일 규칙, 선택 요일, 500m/1km 반경, Always 및 Full Accuracy
...
**Scale/Scope**: 기기 소유자 1명, 활성 규칙 최대 1개, 선택 요일 최대 7개, 원형 위치 조건 1개,
운영체제가 허용하는 선택 앱 토큰 집합 1개, 외부 계정·서버·동기화 없음
