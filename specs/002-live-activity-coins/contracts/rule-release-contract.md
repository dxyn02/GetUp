...
T050 재조정 계약: 동일 로컬 잠금 아래 원격 명령의 식별자·자금 원천과 로컬 예외의 소유권을
확인한다. 예약·적용·결과 불명 상태는 최신 제한 재평가 성공 후 해당 명령의 예외가 있으면 확정,
없으면 보상한다. 보상 중 상태는 자신의 예외만 제거하고 재평가한 뒤 보상을 마친다. 이미 확정된
명령은 추가 차감·보상을 하지 않고, 정리된 예외를 되살리지 않는다. 조회·적용·원격 확정 실패는
반대 금융 연산을 추측 실행하지 않고 재조정 필요로 남긴다. `reconcilePending`은 중복 ID를 한 번만
